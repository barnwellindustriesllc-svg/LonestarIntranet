...
function get_nickelrock_review_rows(mysqli $mysqli, string $weekStart): array {
  ...
  $sql = "
    SELECT n.*,
     ...
     ORDER BY n.work_date, n.provider_name, n.truck_digits, n.ticket_number, n.id
  ";
...
function build_nickelrock_invoice_xlsx(array $rows): string {
  $out = [['Delivery Date','Provider Name','Ticket','Tons','Driver','Truck','Rate','Pay']];
  foreach ($rows as $row) {
    $r = count($out) + 1;
    $out[] = [
      ...
      (float)($row['rate'] ?? 0),
      ['formula' => "D{$r}*G{$r}", 'value' => nickelrock_total_pay($row)],
    ];
  }
  $totalRow = count($out) + 1;
  $lastDataRow = max(2, $totalRow - 1);
  $out[] = ['', '', 'Tons', ['formula' => "SUM(D2:D{$lastDataRow})", 'value' => array_sum(array_map(static fn($r) => (float)($r['tons'] ?? 0), $rows))], '', '', 'Total', ['formula' => "SUM(H2:H{$lastDataRow})", 'value' => array_sum(array_map(static fn($r) => nickelrock_total_pay($r), $rows))]];
  ...
  if ($vendorNumber !== '') $out[] = ['', 'Vendor -' . $vendorNumber];
...
function nextier_total_pay(array $row): float {
  return round((float)($row['line_haul'] ?? 0) - (float)($row['fsc_total'] ?? 0) + (float)($row['bonus'] ?? 0), 2);
}
...
function get_driver_fuel_surcharge_rows(mysqli $mysqli, string $weekStart): array {
  ...
  $sql = "
    SELECT {$driverIdExpr} AS driver_contact_id,
           ...
           SUM({$milesExpr} * COALESCE(ldr.fuel_surcharge_rate, 0)) AS total_fuel_surcharge
      FROM ls_detail_raw ldr
      LEFT JOIN (
  ...
  if (!$files || empty($files['name']) || !is_array($files['name'])) {
    ...
    for ($i = 0; $i < $count; $i++) {
      ...
      $stmt = $mysqli->prepare("
        ...
        (upload_date, work_date, provider_name, ticket_number, tons, driver_name, truck_raw, truck_digits, rate, total_amount, work_order, vendor_number, matched_contact_id)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
        ON DUPLICATE KEY UPDATE
...
if (($_SERVER['REQUEST_METHOD'] === 'POST') && in_array(($_POST['action'] ?? ''), ['add_nickelrock_row', 'save_nickelrock_row'], true)) {
  ...
  if ($driverName === '') $errors[] = 'Enter or select a driver.';
  if ($totalAmount <= 0 && $tons > 0 && $rate > 0) {
    $totalAmount = round($tons * $rate, 2);
  }
      ...
      if ($isAdd) {
        $stmt = $mysqli->prepare("
          ...
          ON DUPLICATE KEY UPDATE
            ...
            matched_contact_id=VALUES(matched_contact_id)
        ");
        if ($stmt) $stmt->bind_param('ssssdsssddssi', $today, $workDate, $providerName, $ticketNumber, $tons, $driverName, $truckRaw, $truckDigits, $rate, $totalAmount, $workOrder, $vendorNumber, $contactIdBind);
      } else {
        $stmt = $mysqli->prepare("
          UPDATE nickelrock_payout_rows
           ...
           LIMIT 1
        ");
        if ($stmt) $stmt->bind_param('sssdsssddssii', $workDate, $providerName, $ticketNumber, $tons, $driverName, $truckRaw, $truckDigits, $rate, $totalAmount, $workOrder, $vendorNumber, $contactIdBind, $rowId);
      }
...
if (($_SERVER['REQUEST_METHOD'] === 'POST') && (($_POST['action'] ?? '') === 'export_nickelrock_invoice')) {
  ...
  if (!$exportRows) {
    ...
    $content = build_nickelrock_invoice_xlsx($exportRows);
    $workOrder = trim((string)($exportRows[0]['work_order'] ?? ''));
    $suffix = $workOrder !== '' ? $workOrder : str_replace('-', '', $weekStartExport);
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment; filename=nickel_rock_' . preg_replace('/[^A-Za-z0-9_-]+/', '_', $suffix) . '.xlsx');
    echo $content;
    exit;
  }
}
      ...
      if (!$insRaw || !$insPayout) {
        ...
        foreach ($sheetNames as $sheetIndex => $sheetName) {
          ...
          for ($rowIndex = $headerIndex + 1; $rowIndex < count($all); $rowIndex++) {
            ...
            $totalPay = round($lineHaul - $fscTotal + $bonus, 2);
            $ticketNumber = $loadId;
...
$selectedUploadVendor = in_array(($_GET['vendor'] ?? ''), ['tss', 'detmar', 'nextier', 'rtex', 'nickelrock'], true) ? (string)$_GET['vendor'] : 'tss';
...
<body>
  ...
  <div class="page-shell">
    ...
    <nav class="vendor-tabs" aria-label="Upload vendor">
      <?php
        $uploadVendorTabs = [
          'tss' => 'TSS',
          'detmar' => 'Detmar',
        ...
        <div class="row g-3 align-items-end">
          ...
          <div class="col-12">
            <div class="small text-muted">
              ...
              Total fuel surcharge is calculated as total miles multiplied by the fuel surcharge rate on each load, then grouped by driver.
            </div>
          </div>
        </div>
      </form>
    ...
    <div class="mt-3 vendor-form d-none" data-vendor="detmar">
      <?php render_vendor_broker_fee_form($vendorBrokerFeeSettings['detmar']); ?>
      <div class="alert alert-info">Detmar upload form is coming soon.</div>
    </div>
    ...
    <div class="mt-3 vendor-form d-none" data-vendor="nickelrock" id="nickelrock-section">
      ...
      <div class="modal fade" id="nickelrockEntryModal" tabindex="-1" aria-labelledby="nickelrockEntryModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
          ...
          <div class="col-6 col-md-1">
            ...
            <input type="number" step="0.01" name="rate" value="9.00" class="form-control" required>
          </div>
          ...
          <div class="col-6 col-md-2">
            ...
            <input type="text" name="work_order" value="53405" class="form-control">
          </div>
          <div class="col-6 col-md-2">
      ...
      <form method="post" enctype="multipart/form-data" class="border rounded p-3 mt-3">
        ...
        <div class="row g-3 align-items-end">
          ...
          <div class="col-6 col-md-1">
            <label class="form-label">Rate</label>
            <input type="number" step="0.01" name="rate_default" value="9.00" class="form-control">
          </div>
          <div class="col-6 col-md-2">
            <label class="form-label">Work Order</label>
            <input type="text" name="work_order_default" value="53405" class="form-control">
          </div>
          <div class="col-6 col-md-2">
            <label class="form-label">Vendor #</label>
      ...
      <?php else: ?>
        <div class="d-flex gap-2 mb-2">
          <form method="post">
            ...
            <button type="submit" class="btn btn-success btn-sm">Export Nickel Rock Invoice</button>
          </form>
            ...
            <tbody>
              <?php foreach ($nickelrockReviewRows as $index => $nickelRow): ?>
                ...
                <tr class="<?= $nickelUnmatched ? 'table-warning' : '' ?>">
                  ...
                  <td><input form="<?= h($nickelFormId) ?>" type="number" step="0.01" name="rate" value="<?= h(number_format((float)$nickelRow['rate'], 2, '.', '')) ?>" class="form-control form-control-sm"></td>
                  ...
                  <td><input form="<?= h($nickelFormId) ?>" type="text" name="work_order" value="<?= h($nickelRow['work_order']) ?>" class="form-control form-control-sm"></td>
    ...
    <div class="mt-3 vendor-form d-none" data-vendor="nextier">
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <div class="row g-3 align-items-end">
          ...
          <div class="col-12">
            <div class="small text-muted">
              NexTier rows are saved to driver payouts using Total Line Haul minus FSC Total plus Bonus. FSC is itemized separately in reporting.
            </div>
          </div>
        </div>
      </form>
